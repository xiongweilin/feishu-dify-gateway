...
- Optional Administrative M6 compatibility forwarding is disabled unless both `ADMINISTRATIVE_INGRESS_BASE_URL` and a transport secret are set. The secret should be supplied through `ADMINISTRATIVE_INGRESS_SHARED_SECRET_FILE` (the default reads `/run/secrets/administrative_ingress_shared_secret` from the external `feishu_secrets` volume); the direct `ADMINISTRATIVE_INGRESS_SHARED_SECRET` environment variable remains available for isolated tests. When enabled, a message whose transport prefix matches `ADMINISTRATIVE_ROUTE_PREFIX` (default `/admin`) is sent only as a reconstructed Feishu metadata envelope plus an internal transport-authentication header to `/v1/intake/feishu/events`; the gateway does not interpret the remainder or dispatch that event to the control plane. Messages without the prefix retain the existing control-plane path.
